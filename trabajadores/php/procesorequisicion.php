	...
	for ($i=0; $i < count($productos); $i++) {
		...
		if (!(empty($productos[$i]))) {
			...
			if (!(empty($cantidades[$i]))) {
				if ($insert == 0) {
					//inserta el pedido
					$sql = "INSERT INTO requisicion(ID_REQUISICION, OBSERVACION_REQUISICION, FECHA_REQUISICION, CI_RIF)
				...
				$fecha=date('Y-m-d g:i:s-a');
				$auditoria="INSERT INTO auditoria_usuarios(ID_AUDITORIA,CI_RIF,OPERACION,DETALLES_OPERACION,FECHA) VALUES('','".$_SESSION['CEDULA']."','Requisicion','Requisicion de Mercancia','$fecha')";
				mysqli_query($conexion, $auditoria);
			}
		}
	}
